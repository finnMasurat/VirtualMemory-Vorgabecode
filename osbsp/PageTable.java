...
public class PageTable {
	...
	private PageTableEntry randomAlgorithm(PageTableEntry newPte) {
		PageTableEntry pte; // Auswahl
		int pageTableSize = pteRAMlist.size();

		Random rand = new Random();
		int randomNum = rand.nextInt(pageTableSize - 0);

		pte = (PageTableEntry) pteRAMlist.get(randomNum);
		os.testOut("Prozess " + pid + ": randomAlgo hat pte ausgew�hlt: "
				+ pte.virtPageNum);
		pteRAMlist.remove(randomNum);
		pteRAMlist.add(randomNum,newPte);
		return pte;
	}
